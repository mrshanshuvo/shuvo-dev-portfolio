...
export async function POST(req: Request) {
  try {
    ...
    const lastItem = await Testimonial.findOne().sort({ order: -1 });
    const order = lastItem ? lastItem.order + 1 : 0;

    const newItem = await Testimonial.create({ ...body, order });
  ...
  try {
    ...
    if (Array.isArray(body)) {
      const updates = body.map((item, idx) =>
        Testimonial.findByIdAndUpdate(item._id, { order: idx }),
      );
