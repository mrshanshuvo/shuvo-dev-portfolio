...
export default function AdminEducationPage() {
  ...
  const [newDetailInputs, setNewDetailInputs] = useState<Record<number, string>>({});
  ...
  function addDetail(i: number) {
    ...
    setEducation((prev) => {
      const next = [...prev];
      next[i].details = [...(next[i].details || []), val];
      return next;
    });
  ...
  function removeDetail(eduIdx: number, detailIdx: number) {
    setEducation((prev) => {
      const next = [...prev];
      next[eduIdx].details = next[eduIdx].details.filter((_, idx) => idx !== detailIdx);
      return next;
    });
  }
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="relative">
            ...
            <p className="text-slate-400 font-medium">
              Curate a world-class academic profile for your portfolio.
            </p>
          </div>
          <div className="flex items-center gap-4">
             <Badge variant="outline" className="bg-blue-500/10 text-blue-400 border-blue-500/20 px-4 py-1.5 rounded-full font-bold uppercase tracking-widest text-[10px]">
              {education.length} Institutions
            </Badge>
            <Button
              onClick={handleSave}
              disabled={saving}
              className="bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold px-8 h-12 shadow-lg shadow-blue-600/20 active:scale-95 transition-all group"
            >
              <FaSave className={cn("mr-2 transition-transform duration-500", saving ? "animate-spin" : "group-hover:rotate-12")} />
              {saving ? "Syncing..." : "Save Education"}
            </Button>
          </div>
        </header>

        <Card className="rounded-[2.5rem] border border-white/10 bg-slate-900/40 backdrop-blur-xl overflow-hidden shadow-2xl">
          <CardHeader className="p-8 md:p-10 border-b border-white/5 bg-white/2">
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription className="text-slate-500 font-medium">
                  Reorder and manage your degrees with premium data fields.
                </CardDescription>
              </div>
              <Button
                variant="outline"
                onClick={addEdu}
                className="bg-slate-800 hover:bg-slate-700 text-white border-white/10 rounded-2xl h-12 px-6 active:scale-95 transition-all"
              >
                <FaPlus size={14} className="mr-2 text-blue-400" /> Add Institution
              </Button>
            </div>
          </CardHeader>
            ...
            <AnimatePresence mode="popLayout">
              {loading ? (
                ...
                Array.from({ length: 2 }).map((_, i) => (
                  <div key={i} className="p-8 bg-slate-950/20 rounded-[2rem] border border-white/5 space-y-6 animate-pulse mb-8">
                ...
                <div className="text-center py-24 bg-slate-950/20 rounded-[3rem] border border-dashed border-white/10">
                  <div className="w-24 h-24 bg-slate-900 rounded-3xl flex items-center justify-center mx-auto mb-6 text-slate-700">
                    <FaGraduationCap size={48} />
                  </div>
                  <h3 className="text-2xl font-bold text-white mb-2">Build Your Foundation</h3>
                  <p className="text-slate-500 mb-10 max-w-sm mx-auto">Display your academic background with the detail it deserves.</p>
                  <Button onClick={addEdu} className="bg-blue-600/10 text-blue-400 hover:bg-blue-600/20 border border-blue-500/20 rounded-2xl px-10 h-14 font-bold text-lg">
                ...
                education.map((edu, i) => (
                  <motion.div
                    ...
                    exit={{ opacity: 0, scale: 0.95 }}
                    className="group relative p-8 md:p-10 bg-slate-950/40 rounded-[2.5rem] border border-white/5 hover:border-white/10 transition-all mb-10 overflow-hidden"
                  >
                    {/* Background Accent */}
                    ...
                    <div className="flex flex-col lg:flex-row gap-10">
                      ...
                      <div className="w-full lg:w-48 shrink-0 space-y-4">
                        ...
                        <div className="flex items-center justify-center gap-2">
                           <Button
                              ...
                              onClick={() => move(i, "up")}
                              disabled={i === 0}
                              className="h-9 w-9 rounded-xl bg-slate-900 border border-white/5 text-slate-500 hover:text-white disabled:opacity-20"
                            >
                              <FaArrowUp size={12} />
                            </Button>
                            <Button
                      ...
                      {/* Info Grid */}
                      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-6 relative z-10">
                        {/* Degree Name */}
                        <div className="space-y-2.5 md:col-span-2">
                          ...
                          <div className="relative group/input">
                            ...
                            <Input
                              className="bg-slate-900/50 border-white/10 text-white rounded-2xl pl-12 h-14 focus-visible:ring-blue-500/50 focus-visible:bg-slate-900 transition-all font-bold text-lg"
                              value={edu.degree}
                        ...
                        <div className="space-y-2.5">
                          ...
                          <div className="relative group/input">
                            ...
                            <Input
                              className="bg-slate-900/50 border-white/10 text-white rounded-2xl pl-12 h-14 focus-visible:ring-blue-500/50 focus-visible:bg-slate-900 transition-all font-bold"
                        ...
                        <div className="space-y-2.5">
                          ...
                          <div className="relative group/input">
                            <FaMapMarkerAlt className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-600 group-focus-within/input:text-blue-400 transition-colors" />
                            <Input
                        ...
                        <div className="space-y-4 md:col-span-2">
                          ...
                          <div className="flex gap-2">
                             <div className="relative group/input flex-1">
                                ...
                                <Input
                                  ...
                                  onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addDetail(i))}
                          ...
                          <div className="space-y-2">
                            <AnimatePresence mode="popLayout">
                              {edu.details?.map((d, dIdx) => (
                                <motion.div
                                  ...
                                  exit={{ opacity: 0, scale: 0.95 }}
                                  className="flex items-center gap-3 bg-slate-950/40 border border-white/5 rounded-2xl p-4 group/detail"
                                >
                            ...
                            {(!edu.details || edu.details.length === 0) && (
                              <p className="text-center py-4 text-slate-600 text-[10px] uppercase tracking-widest font-black opacity-30 italic">
                        ...
                        <div className="space-y-2.5 md:col-span-2">
                          ...
                          <div className="relative group/input">
                            ...
                            <Input
                              className="bg-slate-900/50 border-white/10 text-white rounded-2xl pl-12 h-14 focus-visible:ring-blue-500/50 focus-visible:bg-slate-900 transition-all font-medium"
                              value={edu.link || ""}
                              onChange={(e) => updateEdu(i, "link", e.target.value)}
                    ...
                    <div className="mt-10 pt-8 border-t border-white/5 flex items-center justify-between">
                      <div className="flex items-center gap-2 text-slate-600 text-[10px] font-bold uppercase tracking-widest">
                        <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" /> Ranked Position #{i + 1}
                      </div>
                      <Button
                        variant="ghost"
                      ...
                      >
                        <FaTrash size={14} className="group-hover:animate-bounce" /> Delete Record
                      </Button>
                    </div>
                  </motion.div>
                ))
