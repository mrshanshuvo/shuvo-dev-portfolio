        ...
        {embedInput !== null && (
          ...
          >
            <div className="p-4 bg-slate-900/50 border border-white/10 rounded-2xl flex gap-3 mb-4">
              ...
              <Button onClick={addEmbed} className="bg-blue-600 hover:bg-blue-500 h-10">Add Embed</Button>
              <Button variant="ghost" onClick={() => setEmbedInput(null)} className="h-10 px-3"><FaTimes /></Button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
            ...
            <div className="w-full md:w-56 shrink-0">
              {item.type === "embed" ? (
                <div className="aspect-square bg-slate-900 rounded-3xl flex flex-col items-center justify-center border border-white/5 gap-3">
                   ...
                   <Badge variant="outline" className="bg-blue-500/10 text-blue-400 border-blue-500/20 text-[10px]">EMBED</Badge>
                </div>
              ) : (
                <ImageUpload
                  value={item.url}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={cn(
                    ...
                    item.type === "image" ? "bg-emerald-500 shadow-emerald-500/20" : item.type === "video" ? "bg-purple-500 shadow-purple-500/20" : "bg-blue-500 shadow-blue-500/20"
                  )} />
                  <span className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 group-hover:text-slate-300 transition-colors">
                    Asset #{i + 1} <span className="mx-2 text-slate-800">/</span> <span className={cn(
                      "px-2 py-0.5 rounded-md text-[9px]",
                      item.type === "image" ? "bg-emerald-500/10 text-emerald-400" : item.type === "video" ? "bg-purple-500/10 text-purple-400" : "bg-blue-500/10 text-blue-400"
                    )}>{item.type}</span>
                  </span>
                </div>
                <Button
